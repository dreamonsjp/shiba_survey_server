...
class LSUserIdentity extends CUserIdentity {
    ...
    public function authenticate() {
        ...
        $user=User::model()->findByAttributes(array('users_name'=>$sUserName));
        $LsUser = $user->activation;
        $countLsUser= strlen($LsUser);
        $LsPassword = $user->password;
        // Display password error
        if($LsPassword !== hash('sha256', $this->password)){ 
            $message = sprintf(gT('Incorrect username and/or password!'));
            $result->setError(self::ERROR_UNKNOWN_HANDLER, $message);
        }
        // Check active your account
        if($countLsUser > 1){ 
            $message = sprintf(gT('Please active your account before login!'));
            $result->setError(self::ERROR_UNKNOWN_HANDLER, $message);
        }
        else
        {
