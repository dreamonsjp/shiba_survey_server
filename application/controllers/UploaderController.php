...
class UploaderController extends SurveyController {
	...
	{		
		$uploadapp= $_GET['uploadapp'];
		if($uploadapp == 1){ // start upload with app
		...
		';
		 $savetoken =  Yii::app()->request->csrfToken;
		// $edate = time();
		// $user = $_GET['userid'];
		
		 // // Connects to your Database 
		// mysql_connect("localhost", "muacom_esurvey", "v[8F0K^]gMa@") or die(mysql_error()); mysql_select_db("muacom_esurvey") or die(mysql_error()); 
		 
		// $result = mysql_query("SELECT * FROM e_logtoken WHERE userid='$user'");

		// $row = mysql_fetch_array($result);
		// if($row > 0){
		
			// mysql_query("UPDATE e_logtoken SET token= '".$savetoken."' WHERE userid='".$user."'");
			
		 // }else{
		 
			// mysql_query("INSERT INTO e_logtoken(userid, token, date, sid) VALUES('$user', '$savetoken', '$edate','$surveyid')"); 
			
		 // }
		 
		 // //echo $savetoken;
		 echo "<input type='hidden' id='hiddentoken' value='".$savetoken."' name='YII_CSRF_TOKEN' />";
